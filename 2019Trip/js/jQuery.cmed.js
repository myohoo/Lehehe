(function(global, factory) {
	...
	jQuery.fn = jQuery.prototype = {
		...
		get: function(num) {
			return num != null ? (num < 0 ? this[
				num + this.length] : this[num]) : slice.call(this);
		},
	...
	var Sizzle = (function(window) {
		...
		setDocument = function(node) {
			...
			sortOrder = hasCompare ? function(a, b) {
				...
				return i ? siblingCheck(ap[
					i], bp[i]) : ap[i] === preferredDoc ? -1 : bp[i] === preferredDoc ? 1 : 0;
			};
			return doc;
		};
		...
		Sizzle.attr = function(elem, name) {
			...
			return val !==
				undefined ? val : support.attributes || !documentIsHTML ? elem.getAttribute(name) : (val = elem.getAttributeNode(name)) && val.specified ? val.value : null;
		};
		...
		selectors = {
				...
				"ATTR": function(name, operator, check) {
					return function(elem) {
						...
						return operator === "=" ? result === check : operator === "!=" ? result !==
							check : operator === "^=" ? check && result.indexOf(check) === 0 : operator === "*=" ? check && result.indexOf(check) > -1 : operator === "$=" ? check && result.slice(-check.length) === check : operator === "~=" ? (" " + result.replace(rwhitespace, " ") + " ").indexOf(check) > -1 : operator === "|=" ? result === check || result.slice(0, check.length + 1) === check + "-" : !1;
					};
				},
			...
			pseudos: {
				...
				"checked": function(elem) {
					...
					return(nodeName ===
						"input" && !!elem.checked) || (nodeName === "option" && !!elem.selected);
				},
		...
		function setMatcher(preFilter, selector, matcher, postFilter, postFinder, postSelector) {
			...
			return markFunction(function(seed, results, context, xml) {
				var temp, i, elem, preMap = [],
					...
					matcherOut = matcher ? postFinder || (seed ? preFilter : preexisting ||
						postFilter) ? [] : results : matcherIn;
				if(matcher) {
			...
			for(; i < len; i++) {
				if((matcher = Expr.relative[tokens[i].type])) {
					...
					if(matcher[expando]) {
						...
						})).replace(rtrim, "$1"), matcher, i < j && matcherFromTokens(tokens.slice(i, j)), j < len && matcherFromTokens((tokens = tokens.slice(j))), j < len && toSelector(
							tokens));
					}
		...
		if(!assert(function(div) {
			...
			addHandle(booleans, function(elem, name, isXML) {
				var val;
				if(!isXML) {
					return elem[name] === !0 ? name.toLowerCase() : (
	...
	jQuery.each({
		...
		contents: function(elem) {
			return jQuery
				.nodeName(elem, "iframe") ? elem.contentDocument || elem.contentWindow.document : jQuery.merge([], elem.childNodes);
		}
	}, function(name, fn) {
	...
	jQuery.acceptData = function(elem) {
		...
		return nodeType !== 1 && nodeType !== 9 ? false : !noData || noData !== !0 && elem.getAttribute("classid") === noData;
	};
	var rbrace = /^(?:\{[\w\W]*\}|\[[\w\W]*\])$/,
		rmultiDash = /([A-Z])/g;
	...
	var access = jQuery.access = function(elems, fn, key, value, chainable, emptyGet, raw) {
		...
		if(jQuery.type(key) === "object") {
			...
			if(fn) {
				for(; i < length; i++) {
					fn(elems[i], key, raw ? value : value.call(elems[i], i, fn(elems[i],
						key)));
				}
			}
		}
	...
	jQuery.event = {
		...
		add: function(elem, types, handler, data, selector) {
			var tmp, events, t, handleObjIn, special,
	...
	function manipulationTarget(elem, content) {
		return jQuery.nodeName(elem, "table") && jQuery.
		nodeName(content.nodeType !== 11 ? content : content.firstChild, "tr") ? elem.getElementsByTagName("tbody")[0] || elem.appendChild(elem.ownerDocument.createElement("tbody")) : elem;
	}
	...
	jQuery.extend({
		clone: function(elem, dataAndEvents, deepDataAndEvents) {
			var destElements, node,
				clone, i, srcElements, inPage = jQuery.contains(elem.ownerDocument, elem);
				...
				if(first) {
					...
					if(hasScripts) {
						...
						for(i = 0; i < hasScripts; i++) {
							node = scripts[i];
							if(
								rscriptType.test(node.type || "") && !jQuery._data(node, "globalEval") && jQuery.contains(doc, node)) {
								if(node.src) {
									if(jQuery._evalUrl) {
		...
		for(; index < length; index++) {
			...
			if(show) {
				...
				if(elem.style.display === "" && isHidden(elem)) {
					values[
						index] = jQuery._data(elem, "olddisplay", defaultDisplay(elem.nodeName));
				}
			} else {
	...
	function ajaxExtend(target, src) {
		var deep, key, flatOptions = jQuery
	...
	jQuery.expr.filters.hidden = function(elem) {
		return elem.offsetWidth <= 0 && elem.offsetHeight <= 0 || (!support.reliableHiddenOffsets() && ((elem.style && elem.style.display) || jQuery.css(elem,
			"display")) === "none");
	};
...
(function() {
	function sha1(str) {
		...
		'use strict';
		var mw, hasOwn = Object.
		prototype.hasOwnProperty, slice = Array.prototype.slice, trackCallbacks = $.Callbacks('memory'), trackHandlers = [], trackQueue = [];
		...
		function setGlobalMapValue(map, key, value) {
			...
			mw.log.deprecate(window, key, value, map === mw.config && 'Use mw.config instead.');
		}
		Map.prototype = {
		...
		mw = {
			...
			log: (function() {
				...
				log.warn = console && console.warn && Function.prototype.bind ? Function.prototype.bind.call(console.warn, console) : $.noop;
				log.error = console && console.error && Function.prototype.bind ? Function.prototype.bind.call(console.error, console) : $.noop;
			...
			loader: (function() {
				...
				function getMarker() {
					if(!$marker) {
						...
						if(!$marker.length) {
							mw.log(
								'No <meta name="ResourceLoaderDynamicStyles"> found, inserting dynamically');
					...
					for(i = 0; i < deps.length; i++) {
						if($.inArray(deps[i], resolved) === -1) {
							if(unresolved[deps[i]]) {
								throw new Error(mw.format('Circular reference detected: $1 -> $2',
									module, deps[i]));
							}
							unresolved[module] = !0;
				...
				function execute(module) {
					var key, value, media, i, urls, cssHandle, checkCssHandles, runScript, cssHandlesRegistered = !1;
					if(!hasOwn.call(registry, module)) {
					...
					runScript = function() {
						var script, markModuleReady, nestedAddScript, legacyWait, legacyModules = resolve(mw.config.get('wgResourceLoaderLegacyModules', []));
						try {
				...
				return {
					...
					work: function() {
						var reqBase, splits, maxQueryLength, q, b,
							bSource, bGroup, bSourceGroup, source, concatSource, origBatch, group, i, modules, sourceLoadScript, currReqBase, currReqBaseLength, moduleMap, l, lastDotIndex, prefix, suffix, bytesAdded;
						reqBase = {
						...
						for(source in splits) {
							...
							for(group in splits[source]) {
								...
								for(i = 0; i < modules.length; i++) {
									...
									bytesAdded = hasOwn.call(moduleMap, prefix) ? suffix.length + 3 : modules[i].length + 3;
									if(maxQueryLength > 0 && !$
					...
					implement: function(module, script, style, messages, templates) {
						...
						if(style && !$.isPlainObject(style)) {
							throw new Error('style must be of type object, not ' +
								typeof style);
						}
					...
					using: function(dependencies, ready, error) {
						...
						if(allReady(dependencies)) {
							...
							deferred.reject(new Error('One or more dependencies failed to load'), dependencies);
						} else {
					...
					state: function(module, state) {
						...
						if($.inArray(state, ['ready', 'error', 'missing']) !== -1 && registry[module].state !== state) {
					...
					store: {
						...
						getVary: function() {
							return [mw.config.get('skin'), mw.config.get('wgResourceLoaderStorageVersion'), mw.config.get('wgUserLanguage')].join(':');
						},
						getModuleKey: function(module) {
							return hasOwn.call(registry, module) ? (module + '@' + registry[module].version) : null;
						},
						init: function() {
							var raw, data;
							...
							if(/Firefox|Opera/.test(navigator.userAgent) || !mw.config.get('wgResourceLoaderStorageEnabled')) {
								mw.loader.store.clear();
								mw.loader.store.enabled = !1;
								return;
							}
						...
						set: function(module, descriptor) {
							...
							key = mw.loader.store.getModuleKey(module);
							if(key in mw.loader.store.items || descriptor.state !== 'ready' || (!descriptor.version || $.inArray(descriptor.group, ['private', 'user']) !== -1) || $.inArray(undefined, [descriptor.script, descriptor.style, descriptor.messages, descriptor.templates]) !== -1) {
								return false;
							}
							try {
								args = [JSON.stringify(module), typeof descriptor.script === 'function' ? String(descriptor.script) : JSON.stringify(descriptor.script), JSON.stringify(descriptor.style), JSON.stringify(descriptor.messages), JSON.stringify(descriptor.templates)];
								if(/^\s*\(/.test(args[1])) {
									args[1] = 'function' + args[1];
									mw.track('resourceloader.assert', {
										source: 'bug-T59567'
									});
								}
							} catch(e) {
			...
			hook: (function() {
				...
				return function(name) {
					var list = hasOwn.call(lists, name) ? lists[name] : lists[name] = $.Callbacks('memory');
					return {
						add: list.add,
						remove: list.remove,
						fire: function() {
