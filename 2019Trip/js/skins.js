if("undefined" == typeof jQuery) throw new Error("Bootstrap's JavaScript requires jQuery"); + function(a) {
	...
	a.fn.emulateTransitionEnd = function(b) {
		...
		a.support.transition = b(), a.support.transition && (a.event.special.bsTransitionEnd = {
			...
			handle: function(b) {
				return a(b.target).is(this) ? b.handleObj.
	...
	c.VERSION = "3.3.4", c.TRANSITION_DURATION = 600, c.DEFAULTS = {
		...
		return this
			.$items = a.parent().children(".item"), this.$items.index(a || this.$active)
	...
	function c(b) {
		return this.each(function() {
			...
			!e && f.toggle && /show|hide/
				.test(b) && (f.toggle = !1), e || c.data("bs.collapse", e = new d(this, f)), "string" == typeof b && e[b]()
		})
	}
	var d = function(b, c) {
	...
	g.VERSION = "3.3.4", g.prototype.toggle = function(d) {
		...
		if(!e.is(".disabled, :disabled")) {
			...
			if(b(), !g) {
				...
				documentElement && !f.closest(".navbar-nav").length && a('<div class="dropdown-backdrop"/>').insertAfter(a(this)).on("click", b);
				var h = {
					relatedTarget: this
				};
	...
	var c = function(b, c) {
		this.options = c, this.$body = a(document.body), this.$element = a(b), this.$dialog = this.$element.find(".modal-dialog"), this.$backdrop = null, this.isShown = null, this.originalBodyPad = null, this.scrollbarWidth = 0, this.ignoreBackdropClick = !1, this.options.remote && this.$element.find(".modal-content").load(this.options.remote, a.proxy(
			function() {
	...
	c.VERSION = "3.3.4", c.TRANSITION_DURATION = 300, c.BACKDROP_TRANSITION_DURATION = 150, c.DEFAULTS = {
		...
		this.$element.trigger(e), this.isShown || e.isDefaultPrevented() || (this.isShown = !0, this.checkScrollbar(), this.setScrollbar(), this.$body.addClass("modal-open"), this.escape(), this.resize(), this.$element.on("click.dismiss.bs.modal", '[data-dismiss="modal"]', a.proxy(this.hide, this)), this.$dialog.on("mousedown.dismiss.bs.modal", function() {
			...
			d.$element.parent().length || d.$element.appendTo(d.$body), d.$element.show().scrollTop(0), d.adjustDialog(), e && d.$element[0].
		...
		a(document).off("focusin.bs.modal").on("focusin.bs.modal", a.proxy(function(a) {
			this.$element[0] ===
				a.target || this.$element.has(a.target).length || this.$element.trigger("focus")
		}, this))
		...
		if(this.isShown && this.options.backdrop) {
			...
			if(this.$backdrop = a('<div class="modal-backdrop ' + e + '" />').appendTo(this.$body), this
	...
	a.fn.modal = b, a.fn.modal.Constructor = c, a.fn.modal.noConflict = function() {
		...
		"click.bs.modal.data-api", '[data-toggle="modal"]',
		function(c) {
			var d = a(this),
				e = d.attr("href"),
	...
	c.VERSION = "3.3.4", c.TRANSITION_DURATION = 150, c.prototype.show =
		function() {
			var b = this.element,
			...
			function f() {
				g.removeClass("active").find("> .dropdown-menu > .active").removeClass("active").end().find('[data-toggle="tab"]').attr("aria-expanded", !1), b.addClass("active").find('[data-toggle="tab"]').attr("aria-expanded", !0), h ? (b[0].offsetWidth, b.addClass("in")) : b.removeClass("fade"), b.parent(".dropdown-menu").length && b.closest("li.dropdown").addClass("active").end().find(
	...
	var c = function(b, d) {
		this.options = a.extend({}, c.DEFAULTS, d), this.$target = a(this.options.target).on("scroll.bs.affix.data-api", a.proxy(this.checkPosition, this)).on("click.bs.affix.data-api", a.proxy(this.checkPositionWithEventLoop, this)), this.
		$element = a(b), this.affixed = null, this.unpin = null, this.pinnedOffset = null, this.checkPosition()
	};
...
(function(mw, $) {
	...
	mw.ugcwikiutil = {
		...
		openDialog: function(option) {
			mw.joymedialoghtml = '' + '<div class="joyme-dialog-popup-mc">' + '<div class="joyme-dialog-wiki-popup">' + '<div class="joyme-dialog-warning"><p><span>!</span><b>提示</b></p></div>' + '<div class="joyme-dialog-warn-con">';
			if(option.message) {
			...
			mw.joymedialoghtml += '' +
				'<div class="joyme-dialog-warn-close">' + '    <p class="joyme-dialog-close-btn"><a class="joyme-dialog-confirm">确定</a>';
			if(option.dialogtype == "confirm") {
				mw.joymedialoghtml += '<a class="joyme-dialog-cancel">取消</a>';
			}
			mw.joymedialoghtml += '</p>' + '</div>' + '</div>' + '</div>' + '</div>';
		...
		clickCloseDialog: function(option) {
			$('.joyme-dialog-confirm').unbind("click").click(function() {
				...
				if(mw.closeCallback && typeof mw.closeCallback == "function") {
					mw.closeCallback('accept');
				}
			});
			$('.joyme-dialog-cancel').unbind("click").click(function() {
				...
				$('.joyme-dialog-popup-mc').css("display", "none");
				if(mw.closeCallback && typeof mw.closeCallback == "function") {
					mw.closeCallback('cancel');
				}
			});
		},
		closeDialog: function() {
			clearInterval(mw.autotime);
			if($('.joyme-dialog-popup-mc').is(':visible')) {
				$('.joyme-dialog-popup-mc').css("display", "none");
				if(mw.closeCallback && typeof mw.closeCallback == "function") {
					mw.closeCallback('close');
				}
			}
		},
		autoCloseDialog: function(msg, ttl) {
			mw.ugcwikiutil.closeDialog();
			mw.closetime = typeof ttl !== 'undefined' && ttl > 0 ? ttl + 1 : 3;
...
jQuery(document).ready(function() {
	...
	function sendregverifycode(mobile, lsmresponse, sendtype) {
		if(!mobile) {
			...
			jQuery.post(mediaWiki.util.wikiScript(), {
				action: 'ajax',
	...
	jQuery('.sendVerifyRegMobileCode').on('click', function() {
		...
		code.addClass('on');
		var time = 60;
		usercenter.validCode = !0;
		if(usercenter.validCode) {
			usercenter.validCode = !1;
	...
	jQuery
		('.sendVerifyMobileCode').on('click', function() {
	...
	mw.joymesiteuserfollow = function() {
		...
		if($('#joymesiteuserfollow').hasClass('gz-done')) {
			mw.ugcwikiutil.confirmDialog('确认取消关注吗？', function(action) {
				if(action == "accept") {
					$.post(mediaWiki.util.wikiScript(), {
						action: 'ajax',
			...
			var l, s, c, a, i, p, u, f, d = e(this).addClass("ps-container"),
				...
				x = function() {
					...
					v.bind("mousedown.perfect-scroll", function(e) {
						...
						v.hasClass("in-scrolling") && v.removeClass(
							"in-scrolling")
					})
				},
				P = function() {
				...
				S = function() {
					...
					e(window).bind("touchstart.perfect-scroll", function() {
						...
						o.pageX = t.pageX, o.pageY = t.pageY, r = (new Date).getTime(), null !== l &&
							clearInterval(l), e.stopPropagation()
...
function(e) {
	function t(t) {
		var o = t || window.event,
			...
			l = 0,
			s = 0;
		return t = e.event.fix(o), t.type = "mousewheel", o.wheelDelta && (n = o.wheelDelta / 120), o.detail &&
			(n = -o.detail / 3), s = n, void 0 !== o.axis && o.axis === o.HORIZONTAL_AXIS && (s = 0, l = -1 * n), void 0 !== o.wheelDeltaY && (s = o.wheelDeltaY / 120), void 0 !== o.wheelDeltaX && (l = -1 * o.wheelDeltaX / 120), r.unshift(t, n, l, s), (e.event.dispatch || e.event.handle).apply(this, r)
	}
...
function isWeiXin() {
	...
	if(ua.match(/MicroMessenger/i) == 'micromessenger') {
		$(".navbar-brand").attr("href", "/xysw/%E5%BE%AE%E4%BF%A1%E6%89%8B%E6%9C%BA%E4%B8%BB%E9%A1%B5");
	}
}
isWeiXin();
var LoadMore = {
	...
	loadComment: function(className, action, rsargs) {
		...
		jQuery.post(mw.util.wikiScript(), {
			action: 'ajax',
			rs: action,
			rsargs: rsargs_val
		}, function(data) {
...
$(function() {
	...
	function changeW(config) {
		var ele = config.ele;
		elePar = ele.parent('.dropdown'), eleW = config.eleW;
		ele.css({
			'width': eleW
		});
	};
	...
	if($(window).width() < 992) {
		$(".joyme-dialog-wiki-popup").css("left", ($(window).width() - $(
			".joyme-dialog-wiki-popup").width()) / 2 + "px");
	}
	...
	function mbindEvent() {
		$('.dropdown-toggle-edit').click(function() {
			window.
			location.href = $('#p-actions>a').attr('href');
		})
	...
	if(typeof $("[data-toggle='popover']").popover == 'function') {
		...
		$('#pc_ad_top').on('click', function() {
			mw.ugcwikiutil.msgDialog($(this).attr('data-content'));
			$('.joyme-dialog-wiki-popup').css({
				'height': 'auto',
				'top': '10%'
			});
		});
	}
	$('#pc_ad_top').trigger('mouseover');
	$('#pc_ad_top').trigger('mouseout');
});
mw.loader.state({
	"skins.mediawikibootstrap": "ready"
});